...
<form method="POST" action="{{ route('authors.update', $authors->id) }}">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
    <div>
      ...
      <input name="first_name" value="{{ $authors->first_name }}" class="w-full border p-2 rounded" />
      {{-- @error('title') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror --}}
    </div>

    <div>
      <label class="block text-sm">Last Name</label>
      <input name="last_name" value="{{ $authors->last_name }}" class="w-full border p-2 rounded" />
